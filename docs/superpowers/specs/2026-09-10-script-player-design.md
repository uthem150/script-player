...
**화자별 음성 배정.** 음성이 하나뿐인 기기가 많다(한국어는 보통 그렇다).
음성이 부족하면 **음높이로 구분**한다 — 화자 순서에 따라 `1.0, 0.85, 1.15, 0.92 …`.
사용자가 설정에서 화자별로 직접 바꿀 수 있다.
...
| 위험 | 영향 | 대응 |
|---|---|---|
| 기기에 오프라인 한국어 음성 없음 | 앱을 쓸 수 없음 | 첫 실행 감지 + 시스템 설정 딥링크. 사전 검증(§11 1단계) |
| 합성이 예상보다 느림 | 첫 재생까지 대기 | 스트리밍 시작으로 첫 소리는 빠름. 실측을 1단계에서 |
| `synthesizeToFile` 부실한 엔진 | 핵심 기능 상실 | live `speak()` 폴백 모드 |
| 에어팟 탭이 안 잡힘 | 주 사용 시나리오 손상 | Media3 `MediaSessionService` 는 정식 경로라 가능성 낮음. 1단계에서 확인 |
| WAV 용량 | 저장 공간 압박 | 500MB 상한 + LRU, `cacheDir` 사용 |
| AGP 가 JDK 25 미지원 | 빌드 불가 | JDK 21 병행 설치 |
| 문장 수백 개 플레이리스트 성능 | 재생 지연 | Media3 는 대용량 플레이리스트를 다루도록 만들어졌음. 1000문장 픽스처로 측정 |
| Kotlin·Compose 학습 | 초기 속도 | 파서(순수 함수)부터 시작해 언어를 익히고 플랫폼 API 는 나중에 |
...
- 합성 실속도 (10분 대본 준비 시간)
- 기본 한국어 음성의 개수와 품질 — 화자 구분을 음성으로 할 수 있는지, 음높이에
  의존해야 하는지
- 1000문장 플레이리스트의 실제 성능
