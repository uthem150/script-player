...
(초안에는 사용자가 조절하는 전역 음높이를 그 위에 곱한다고 적었지만, 그 조절은
만들지 않았다 — §3.6)
...
### 3.6 왜 전역 음높이를 만들지 않았나

스펙 초안에 설정 항목으로 적어 뒀지만 만들지 않았다. TTS 음성의 피치를 전역으로 흔들면
대개 이상하게 들리고, 화자를 가르는 음높이는 이미 합성 시점에 들어가 있다(§3.3).

대신 **마지막 배속을 기억**하게 했다. 늘 1.5배로 듣는 사람이 대본마다 다시 누르는 것이
실제로 겪는 불편이고, 이쪽이 같은 자리에 놓을 값으로 훨씬 낫다.

### 3.7 왜 콘텐츠 주소 지정 캐시인가
...
┌───────────────────────────────────────────────────────┐
│ ui/  (Compose)                                        │
│  library · player · settings · voice · theme · component│
│  화면(상태만 받음) + Route(상태를 붙이는 층)             │
└───────┬───────────────┬──────────────┬────────────────┘
        │               │              │
┌───────▼─────┐  ┌──────▼──────┐  ┌───▼──────────────────┐
│ parser/     │  │ tts/        │  │ playback/            │
│ 순수 Kotlin │  │ 합성 파이프  │  │ Media3 · 조율·진도    │
└─────────────┘  └──────┬──────┘  └───┬──────────────────┘
                        │             │
                  ┌─────▼─────────────▼───────────────┐
                  │ data/  Room · AudioCache · Settings│
                  └───────────────────────────────────┘
```

**`domain/` 은 만들지 않았다.** 초안에서 조율 계층을 따로 두려 했지만, 실제로 조율할
것은 "합성 결과를 재생기에 순서대로 붙이기" 하나였고 그것은 `playback/PlaybackSession`
이 맡는다. 화면과 데이터를 잇는 나머지는 각 `Route` 가 열 줄 안에서 끝난다 — 빈 계층을
하나 더 두면 부르는 곳만 늘어난다.

`share/` 는 `ACTION_SEND` 를 받아 보관함에 넘기는 얇은 진입점이다.
...
| `tts/` | `TextToSpeech` 격리, 문장 큐 합성, 음성 고르기 | 어댑터만 |
| `playback/` | `MediaSessionService`, 조율, 진도 누적합, 단어 탭 환산 | 어댑터만 |
| `data/` | Room 보관함, 콘텐츠 주소 오디오 캐시, 설정 | Room, 파일시스템 |
| `ui/` | 화면 6개 + 디자인 시스템 | Compose |
| `share/` | `ACTION_SEND` 수신 | Intent |

**격리 원칙.** `parser/` 는 안드로이드 클래스를 하나도 import 하지 않는다.
`tts/` 와 `playback/` 은 각각 `Synthesizer`, `PlayerController` 인터페이스 뒤에 플랫폼을
숨겨, 조율 로직(`PlaybackSession`·`SynthesisQueue`)과 계산(`SentenceTimeline`·`WordSeek`·
`AudioCache`)이 기기 없이 테스트된다. 실기기가 필요한 것은 어댑터 두 개
(`AndroidSynthesizer`·`Media3PlayerController`)뿐이다.
...
data class Sentence(
    ...
    val sourceRange: IntRange,  // 원문 오프셋 — 원문으로 돌아가는 다리 (§5.1 아래 참고)
)
```
...
`sourceRange` 는 정리 전 원문 기준으로 보존한다.

**구현하며 바뀐 것.** 처음에는 화면이 원문을 그대로 보여주고 그 위에 하이라이트를 얹을
생각이었다. 그러면 사용자가 `**` 와 `##` 를 그대로 보게 되는데, 들으면서 따라 읽는
화면에서 그것은 방해다. 화면은 **정리된 문장**을 보여준다. 따라서 `sourceRange` 는
하이라이트의 기준이 아니라 **원문으로 돌아가는 다리**다 — 대본을 고칠 때 쓴다.
...
음성마다 다를 수 있다. **오프셋 24 로 못 박지 말고 청크를 훑어 `"fmt "` 청크를 찾는다** —
엔진이 청크를 더 끼워 넣으면 엉뚱한 4바이트를 샘플레이트로 읽는다.
(실측: Google TTS 한국어 = 24000Hz)

**매니페스트에 `queries` 가 필요하다.** Android 11+ 의 패키지 가시성 때문에, 아래가 없으면
설치된 TTS 엔진 조회 결과가 빈다.
...
@Entity data class ScriptEntity(
    ...
    val createdAt: Long,
    val updatedAt: Long,
    val sentenceCount: Int,      // 목록에 필요해 함께 둔다
    val lastSentenceIndex: Int,  // 이어듣기
    val lastPositionMs: Long,
)

**화자별 설정은 여기 두지 않았다.** 처음에는 `speakerSettingsJson` 을 대본마다 둘
생각이었는데, 화자 id 가 등장 순서(`s0`·`s1`…)라 어느 대본이든 "첫 화자는 이 목소리" 가
성립한다. 자리별 전역 설정([AppSettings])으로 두면 사용자가 대본마다 다시 정할 필요가
없고 Room 이주도 필요 없다.

목록 조회는 `raw` 를 뺀 열만 골라 받는다 — 대본 수십 편의 본문을 목록 그릴 때마다
메모리로 끌어올릴 이유가 없다.
```
...
- 같은 키의 `.timings` 에 **단어 타이밍 표**를 함께 둔다 (§5.2). 오디오와 타이밍은
  같은 합성의 산물이라 수명을 함께 가야 한다 — 따로 관리하면 한쪽만 남아 어긋난다.
  (처음에 `.json` 으로 적었지만 정수 셋이 한 줄인 표라 JSON 파서를 끌어올 이유가 없어
  쉼표로 나눈 평문으로 뒀다)
- 줄 하나라도 망가져 있으면 **타이밍 전체를 버린다.** 절반만 맞는 표로 단어를 짚으면
  엉뚱한 자리에서 재생되는데, 그것은 없는 것보다 나쁘다
- 총량 상한 **500MB**, 넘으면 접근 시각 오래된 것부터 삭제 (`.wav` 와 `.timings` 를 한 쌍으로)
...
- 화자 자리별 음성 고르기 (음높이는 음성이 부족할 때만 자동으로 갈라 준다) — 변경하면 해당 화자 문장만 재합성 (§3.6)
- 마지막 배속 기억 (전역 음높이는 만들지 않았다 — 아래 참고)
...
| `synthesizeToFile` 을 엔진이 아예 지원 안 함 | **아직 대응하지 않았다** (아래 참고) |
...
**live `speak()` 폴백 모드는 만들지 않았다.**

실측 기기에서 `synthesizeToFile` 이 확인됐고(§12), 폴백은 시크·실시간 속도가 없는 두 번째
재생 경로를 통째로 더하는 일이다. `PlayerController` 인터페이스가 이미 두 구현을 받을 수
있게 되어 있으니 필요한 기기가 나타나면 그때 붙인다 — 가지고 있지도 않은 기기를 위해
지금 그 복잡도를 지불하지 않는다.

그래서 스크린샷 축에도 «폴백 모드» 가 없다.
...
- 화면: 보관함(빈 상태·목록), 재생기(재생 중·정지·합성 중), 새 대본(빈 상태·읽을 문장 없음),
  공유 확인, 설정, 음성 없음 안내
- 테마: 밝게 × 어둡게
...
| `synthesizeToFile` 부실한 엔진 | 다른 기기의 위험 | 아직 대응 안 함 — 필요해지면 폴백 (§6) |
...
문장 41개를 실제 플레이리스트로 걸고 다시 쟀다.

- **에어팟 2탭 = 문장 이동 확인.** `버튼·시크로 이동` 이 양방향으로 찍혔다
- **문장 1→26 이 `자동으로 다음` 으로 내내 이어졌다.** 문장별 파일을 플레이리스트로
  태우는 구조가 실제로 도는 것을 확인한 것이라, 설계의 뼈대가 검증됐다
- 합성 속도 재측정 **32.7배** (첫 측정 37.7배). 10분 대본 18초 · WAV 27MB
- 버튼으로 항목을 옮기면 직후에 짧게 `정지`→`재생` 이 찍힌다. **버퍼링이라 정상이다** —
  나중에 버그로 오해하지 않도록 적어 둔다

### 확인하지 않고 넘긴 것

아래는 검증 앱을 더 만들지 않고 **8단계 실기기 체크리스트(§7.3)로 넘긴다.** 어차피 실제 앱에서
같은 코드로 다시 지나가는 자리이고, 여기까지 확인된 것으로 위험이 충분히 낮아졌다.

| 넘긴 것 | 왜 낮게 보는가 |
| --- | --- |
| 화면 끈 뒤 연속 재생 | `MediaSessionService` 는 안드로이드의 정식 경로다. 에어팟 버튼이 들어왔다는 것 자체가 세션이 살아 있다는 뜻이고, 105초 연속 재생도 확인됐다 |
| 비행기 모드 합성 | 쓴 음성이 `ko-kr-x-ism-local`(`isNetworkConnectionRequired=false`)이고 41문장 전부 성공했다 |
| 잠금화면 컨트롤 | Media3 가 알림과 함께 기본으로 제공한다 |
| 1000문장 플레이리스트 성능 | 41문장이 매끄러웠다. 6단계에서 픽스처로 측정 |
